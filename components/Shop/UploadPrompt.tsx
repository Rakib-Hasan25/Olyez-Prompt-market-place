...
export default function UploadPrompt({}: Props) {
  ...
  const handleImageFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      //convert all files into arrays

      files.forEach((file) => {
        const reader = new FileReader();
        // . FileReader is a built-in JavaScript object that allows reading the content of files asynchronously.
        reader.onload = () => {
          // This line sets up an event handler function to run when the file reading process is complete
          // . The onload event is triggered when the file has been successfully loaded into memory by the FileReader object.

          if (reader.readyState === 2) {
            //           Inside the event handler function, this line checks the
            //           readyState property of the FileReader object. The readyState property tells us the current state of the file reading operation.
            // The value 2 of readyState means that the reading operation is complete. So, this line
            // ensures that the file is fully loaded before proceeding with any further actions.

            setPromptData((prevData) =>
              // prevdata is the previous state of Promptdata
              //in here we create a new object where we store the previous state data

              ({
                ...prevData,
                // The spread operator (...) is used to copy all properties of prevData into this new object.

                images: [...prevData.images, reader.result as string],

                // the images property is updated. It's set to a new array that contains the
                // previous images (prevData.images) along with a new image. The new image is
                // reader.result, which is cast to a string (as string).
              })
            );
          }
        };
        ...
        //       This line starts the reading process for the specified
        //        file (file) using the readAsDataURL() method of the FileReader object.
        //        This method reads the content of the file as a data URL.
        // Once the reading process is complete, the onload event handler (defined above)
        //  will be triggered to handle the loaded file content.
      });
    }
  };
...
<input
  ...
  placeholder="Choose image(s) to upload"
/>
        ...
        <br />
        <div className="w-full">
        <label htmlFor="attachment" className="hidden">
    Attachments
  </label>
  <input
    type="file"
